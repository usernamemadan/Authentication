...
// MARK: - properties
    ...
    func getNotes(isArchived: Bool) {
 //       PersistentManager.shared.fetchNotes()

        NetworkManager.shared.fetchNotes(queryType: .initialFetch, archived: isArchived) { result in
            switch result {
            case .failure(let error):
                print("error while fetching notes - \(error)")
...
extension HomeViewController: UIScrollViewDelegate{
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        if offsetY > contentHeight - height{
            ...
            NetworkManager.shared.fetchNotes(queryType: .paginationFetch, archived: showArchivedNotes) { result in
                switch result{
                case .failure(let error):
